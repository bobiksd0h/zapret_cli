...
main_menu() {
    while true; do
        ...
        if [[ $ZAPRET_EXIST == true ]]; then
            ...
            read -p $'\e[1;36mВыберите действие: \e[0m' CHOICE
            case "$CHOICE" in
                1) install_zapret_release; main_menu;;
                0) $TPUT_E; exit 0;;
                *) echo -e "\e[1;31m Неверный ввод! Попробуйте снова.\e[0m"; sleep 2;;
            esac
        fi
...
change_configuration() {
    while true; do
        ...
        case "$CHOICE" in
            ...
            12) open_editor "$ZAPRET_BASE/config" ;;
            13) open_editor "$ZAPRET_BASE/ipset/zapret-hosts-user.txt" ;;
            14) touch "$ZAPRET_BASE/ipset/zapret-hosts-user-exclude.txt"; open_editor "$ZAPRET_BASE/ipset/zapret-hosts-user-exclude.txt" ;;
            15) edit_cust_list;;
            16) edit_cust_conf;;
            0) main_menu ;;
...
update_zapret_menu(){
    while true; do
        ...
        echo -e "║   Текущая версия: $(if [ -f "$VER_FILE" ]; then cat "$VER_FILE"; else echo "Неизвестно";fi)"
        echo -e "║   Последняя версия: $(get_latest_version)"
        echo -e "╚════════════════════════════════════╝\e[0m"
        echo -e "  \e[1;31m0)\e[0m Выйти в меню"
        echo -e "  \e[1;33m1)\e[0m Обновить запрет (бинарники) и переприменить конфигурацию"
        echo ""
        read -p $'\e[1;36m Выберите действие: \e[0m' CHOICE
        case "$CHOICE" in
            1) update_zapret;;
